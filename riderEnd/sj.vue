    ...
    <nav-bar title-align="left" :showBackButton="false">
      <template #title>
        <view style="display: flex; align-items: baseline">
          <text style="font-size: 14px; font-weight: 500; color: #ffffff"
            >骑手端</text
          >
          <text
            style="
    ...
    <view class="orderbox">
      <view class="order-list">
            <view
              ...
              :class="{
                'completed-order': order.isCompleted && order.isRecentTask,
                'assigned-order': order.isAssigned && order.isRecentTask,
              }"
            ...
            >
              ...
              <image
                ...
                style="
                  ...
                  height: 72px;
                "
                src="https://ccpt.qiniu.cc111.cn/已打赏.svg"
                mode="heightFix"
              >
              </image>
              ...
              <view class="order-content">
                ...
                >
                  <view class="icon-content">
                    <text class="brand-text">{{
                      getServiceTypeFirstChar(order.serviceTypeText)
                    }}</text>
                    <text class="service-text"
                      >{{ getBrandText(order.brand)
                      }}{{ order.serviceTypeText }}</text
                    >
                  </view>
                </view>
                ...
                <view class="order-details">
                  ...
                  <view class="service-item" :data-content="order.brand"
                    >品牌要求：</view
                  > 
                  <view
                    class="service-item"
                    :data-content="order.industry"
                    >商机业态：</view
                  >
                </view>
                ...
                <view class="price-info-wrapper">
                  <view
                    class="service-time"
                    :data-content="order.price"
                  ></view>
                </view>
                <view
                  v-if="order.order == null"
                  class="assigned-label"
                  >去抢购</view
                >
                <view v-else class="completed-label">已购买</view>
              </view>
            </view>
      </view>
    </view>
        ...
        <view class="asdasd" style="background-color: #fff; margin-top: -2rpx">
          <view class="order-modal-content">
            <view class="order-modal-info">
              <view class="info-row">
                <text class="info-label">品牌要求：</text>
                <text class="info-value service-item-value">{{
                  currentOrderInfo.brand
                }}</text>
              </view>
              <view class="info-row">
                <text class="info-label">商机业态：</text>
                <text class="info-value amount">{{
                  currentOrderInfo.industry
                }}</text>
              </view>
              <view class="info-row">
                <text class="info-label">商机费用：</text>
                <text class="info-value amount">{{
          ...
          <view class="order-modal-content mar-top-10">
            ...
            <view class="order-modal-info">
              <view class="info-row">
                <text class="info-label">商机位置：</text>
                <text class="info-value">{{
                  formatAddress(currentOrderInfo)
                }}</text>
              </view>
            </view>
          </view>
          ...
          <view class="order-modal-footer">
            <button class="modal-btn cancel" @click="closeOrderModal">
              取消
            </button>
            <button
              class="modal-btn confirm"
              :class="{ disabled: isAcceptingOrder }"
              @click="acceptOrder(currentOrderInfo)"
            >
              <text>去支付</text>
            </button>
          </view>
      ...
      <view class="cancel-modal-container" @click.stop>
        ...
        <view class="cancel-modal-header">
          <image
            ...
            mode="widthFix"
          ></image>
          <text class="cancel-intro-text"
            >您有{{ canceledTasks.length }}个任务已被系统自动撤销：</text
          >
        </view>
        ...
        <view class="cancel-modal-content">
            ...
            <view class="task-item">
              <view class="task-row">
                ...
                <text class="task-value">{{
                  canceledTasks[0].task.task_no
                }}</text>
              </view>
              <view class="task-row">
                <text class="task-label">门店名称</text>
                <text class="task-value">{{
                  canceledTasks[0].task.task_detail.store_name
                }}</text>
              </view>
              <view class="task-row">
                <text class="task-label">撤销时间</text>
                <text class="task-value">{{
                  formatCancelTime(canceledTasks[0].created_at)
                }}</text>
              </view>
              <view class="task-row">
                <text class="task-label">撤销原因</text>
                <text class="task-reason">{{
                  canceledTasks[0].cancel_reason ||
                  "订单超时自接单后36小时未完单"
                }}</text>
              </view>
            </view>
          ...
          <view v-else class="multiple-tasks">
            <scroll-view scroll-y class="task-list">
              <view
                v-for="(task, index) in canceledTasks"
                :key="index"
                class="task-item"
              >
                <view class="task-row">
                ...
                <view class="task-row">
                  <text class="task-label">门店名称</text>
                  <text class="task-value">{{
                    task.task.task_detail.store_name
                  }}</text>
                </view>
                <view class="task-row">
                  <text class="task-label">撤销时间</text>
                  <text class="task-value">{{
                    formatCancelTime(task.created_at)
                  }}</text>
                </view>
                <view class="task-row">
                  <text class="task-label">撤销原因</text>
                  <text class="task-reason">{{
                    task.cancel_reason || "订单超时自接单后36小时未完单"
                  }}</text>
                </view>
              </view>
            </scroll-view>
        ...
        <view class="cancel-modal-footer">
          <view class="modal-btn-new confirm" @click="closeCancelModal"
            >知道了 下次我尽快完成！</view
          >
        </view>
      </view>
...
export default {
  ...
  data() {
    return {
      ...
      bannerList: [
        ...
        {
          id: 3,
          image: "https://ccpt.qiniu.cc111.cn/qs-bn.png",
          title: "Banner 2",
          url: "",
        },
      ],
      ...
      shareCountData: {},
      // 分享图片路径
      shareImageUrl: "https://ccpt.qiniu.cc111.cn/qs-bn.png",
      // 分享参数
      shareParams: null,
      // 当前要转派的订单
  ...
  onPullDownRefresh() {
    const savedCity = uni.getStorageSync("rider_selectedCity");
    uni.setStorageSync("rider_selectedCity", {
      name: "",
      fullCityName: "全部",
    });
    this.currentCity = "全部";
  ...
  async onShareAppMessage() {
    ...
    if (order && order.id) {
      ...
      const shareData = {
        ...
        imageUrl: "https://ccpt.qiniu.cc111.cn/qs-bn.png",
      };
  ...
  methods: {
      ...
      try {
        ...
        if (res.status === "success" && res.data) {
          ...
          const updatedUserInfo = {
            ...
            month_completed_tasks_count:
              res.data.month_completed_tasks_count || 0,
            today_commission: res.data.today_commission || 0,
            today_completed_tasks_count:
              res.data.today_completed_tasks_count || 0,
            ...
            provider_id:
              res.data.service_provider_id ||
              res.data.provider_id ||
              riderUserInfo.provider_id,
          };
      ...
      try {
        ...
        if (res.code === 200 && res.data) {
          const newOrders = res.data.map((item) => {
            ...
            if (
              item.reward &&
              Array.isArray(item.reward) &&
              item.reward.length > 0
            ) {
              const paidRewards = item.reward.filter(
                (r) => r.status === "paid",
              );
              if (paidRewards.length > 0) {
                const totalAmount = paidRewards.reduce(
                  (sum, r) => sum + parseFloat(r.order_amount || 0),
                  0,
                );
                rewardInfo = {
            ...
            return {
              ...
              serviceType: this.getServiceTypeClass(
                item.task_detail?.detail || item.task_name,
              ),
              serviceTypeText: this.getServiceTypeDisplayText(
                item.task_detail?.detail || item.task_name,
              ),
              storeName: item.task_detail?.store_name || "未知店铺",
              serviceTime: this.formatServiceTime(
                item.task_date,
                item.deadline,
                item.time_limit,
              ),
              ...
              taskDuration: this.calculateTaskDuration(
                item.start_date,
                item.task_assignment?.finished_at,
              ),
              ...
              doorImage:
                item.task_detail &&
                item.task_detail.pic_url &&
                item.task_detail.pic_url.length > 0
                  ? item.task_detail.pic_url[0]
                  : "",
              brand: item.brand || "",
              reward: rewardInfo,
    ...
    async getWaitingTasks() {
      ...
      try {
        ...
        const res = await this.$request("business/list", params, "POST");
    ...
    formatOrderData(
      ...
      if (
        order.reward &&
        Array.isArray(order.reward) &&
        order.reward.length > 0
      ) {
        // 计算打赏总金额（只计算已支付的）
        const paidRewards = order.reward.filter(
      ...
      return {
        ...
        serviceType: this.getServiceTypeClass(
          taskDetail.detail || order.task_name,
        ),
        serviceTypeText: this.getServiceTypeDisplayText(
          taskDetail.detail || order.task_name,
        ),
        ...
        refundRequest:
          order.refund_request === 1 || order.refund_request === "1", // 是否为退款中订单
    ...
    getServiceTypeClass(detail) {
      // 处理多选情况，取第一个服务类型
      const firstDetail =
        detail && detail.includes(",") ? detail.split(",")[0].trim() : detail;
    ...
    formatServiceItems(taskDetail, order) {
      ...
      if (taskDetail.detail) {
        // 检查是否是多选（包含逗号）
        const details = taskDetail.detail.includes(",")
          ? taskDetail.detail.split(",")
          : [taskDetail.detail];
    ...
    goToOrderDetail(order) {
      if (!order.order) {
        ...
        // 跳转订单详情页面
        uni.navigateTo({
          url: `/pages/index/publish/sjfb?id=${order.id}`
        });
      } 
    },
        ...
        setTimeout(() => {
          ...
          if (retryOrder) {
            ...
            uni.showModal({
              title: "提示",
              content:
                "未找到对应的转派订单，可能该订单已被其他骑手接单或状态发生变化。",
              showCancel: false,
              confirmText: "知道了",
            });
          }
    ...
    async acceptOrder() {
      ...
      console.log(this.riderUserInfo);
      try {

        const res = await this.$request("business/pay", {
          openid: userInfo.openid,
          user_id: userInfo.user_id,
          business_id: this.currentOrderInfo.id,
        }, "POST");

        if (res.code === 200) {
          const payParams = res.data.pay_res;
          uni.requestPayment({
            provider: "weixin",
            ...
            success: (res) => {
              ...
              setTimeout(() => {
                uni.navigateTo({
                  url: `/pages/index/publish/sjfb?id=${this.currentOrderInfo.id}`
                });
              }, 1500);
            },
            fail: (err) => {
    ...
    transferOrder() {
      try {
        ...
        const orderIndex = this.orderList.findIndex(
          (order) => order.id === this.currentOrderInfo.id,
        );
        if (orderIndex !== -1) {
    ...
    getTotalAmountWithReward(order) {
      ...
      if (order.reward && order.reward.amount) {
        ...
        // 打赏金额乘以费率
        const rewardValue =
          parseFloat(order.reward.amount) *
          (this.riderUserInfo && this.riderUserInfo.rate
            ? Number(this.riderUserInfo.rate)
            : 1);
    ...
    async generateShareImage() {
      return new Promise(async (resolve, reject) => {
        try {
          ...
          const orderCount =
            this.shareCountData && this.shareCountData.waiting_count
              ? this.shareCountData.waiting_count * 2
              : 0;
              ...
              uni.canvasToTempFilePath(
                {
                  ...
                  fail: (err) => {
                    ...
                    this.shareImageUrl =
                      "https://ccpt.qiniu.cc111.cn/qs-bn.png";
          ...
          this.shareImageUrl = "https://ccpt.qiniu.cc111.cn/qs-bn.png";
          resolve(this.shareImageUrl);
        }
      });
    },
          ...
          success: (uploadFileRes) => {
            try {
              ...
              if (res.path) {
                ...
                this.shareImageUrl = "https://ccpt.qiniu.cc111.cn/qs-bn.png";
                resolve("https://ccpt.qiniu.cc111.cn/qs-bn.png");
              }
            } catch (error) {
    ...
    updateCountdown() {
      ...
      const countdownResult = this.calculateTransferCountdown(
        this.currentOrderInfo,
      );
    ...
    calculateCountdown(deadline, startDate) {
      ...
      try {
        ...
        if (startDate) {
          ...
          const durationHours =
            (deadlineDate - startDateTime) / (1000 * 60 * 60);
        ...
        // 计算实际截止时间（提前N小时）
        const actualDeadline = new Date(
          deadlineDate.getTime() - advanceHours * 60 * 60 * 1000,
        );
    ...
    processOrderDataForShare(orderData) {
      try {
        ...
        orderData.forEach((order) => {
          ...
          // 构建地区地址（不包含省份）
          const areaAddress = [order.city_name, order.district_name]
            .filter(Boolean)
            .join("");
...
.order-list {
  ...
  .order-item {
    ...
    .order-content {
      ...
      .order-icon {
        ...
        // 品牌颜色样式
        &.brand-meituan {
          background-image: url("https://ccpt.qiniu.cc111.cn/riderend/meituan-b.png") !important;
        ...
        &.brand-guaishou {
          background-image: url("https://ccpt.qiniu.cc111.cn/riderend/guaishou-b.png") !important;
          background-size: cover !important;
          background-position: center !important;
          background-repeat: no-repeat !important;
          background-color: transparent !important;
        ...
        &.brand-jiedian {
          background-image: url("https://ccpt.qiniu.cc111.cn/riderend/zhumang-b2.png") !important;
          background-size: cover !important;
          background-position: center !important;
          background-repeat: no-repeat !important;
          background-color: transparent !important;
        ...
        &.brand-xiaodian {
          background-image: url("https://ccpt.qiniu.cc111.cn/riderend/xiaodian-b.png") !important;
          background-size: cover !important;
          background-position: center !important;
          background-repeat: no-repeat !important;
          background-color: transparent !important;
  ...
  .completed-stamp-image {
    ...
    z-index: 10;
    pointer-events: none;
    filter: brightness(0) saturate(100%) invert(27%) sepia(99%) saturate(1234%)
      hue-rotate(201deg) brightness(96%) contrast(95%);
  }
  ...
  .assigned-stamp-image {
    ...
    z-index: 10;
    pointer-events: none;
    -webkit-filter: brightness(0) saturate(100%) invert(27%) sepia(99%)
      saturate(1234%) hue-rotate(201deg) brightness(96%) contrast(95%);
    filter: brightness(0) saturate(100%) invert(27%) sepia(99%) saturate(1234%)
      hue-rotate(201deg) brightness(96%) contrast(95%);
  }
}
...
@keyframes glow {
  ...
  to {
    text-shadow:
      0 2rpx 12rpx rgba(255, 215, 0, 0.8),
