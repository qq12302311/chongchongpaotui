<template>
  <view style="min-height: 100vh; background-color: #f8f9fa; display: flex; flex-direction: column; position: relative; width: 100%; overflow: hidden;">
      ...
      <view v-else>
        ...
        >
          ...
          <view style="display: flex; padding: 20rpx 24rpx; gap: 12rpx; border-top: 1rpx solid #f1f3f4; background-color: #fff;">
            ...
            <view style="flex: 1; height: 60rpx; border-radius: 30rpx; display: flex; align-items: center; justify-content: center; font-size: 22rpx; font-weight: 500; border: 1rpx solid #e9ecef; background-color: #ff6b6b; color: #fff; border-color: #ff6b6b;" @click.stop="showStatusModal(user)">状态管理</view>
          </view>
        </view>
      </view>
    ...
    <!-- 状态管理弹窗 -->
    <view class="modal-mask" v-if="showStatusDialog" @click="closeStatusModal"></view>
    <view class="modal-container" v-if="showStatusDialog">
      <view class="modal-header">
        <text class="modal-title">用户状态管理</text>
        <view class="modal-close" @click="closeStatusModal">×</view>
      </view>
      <view class="modal-content">
        <view class="status-options">
          ...
          >
            <view class="status-radio" :class="{ checked: selectedStatus === status.value }"></view>
      ...
      <view class="modal-footer">
        <view class="modal-btn cancel" @click="closeStatusModal">取消</view>
        <view class="modal-btn confirm" @click="updateUserStatus">确认</view>
      </view>
    </view>
...
export default {
  ...
  methods: {
    ...
    showStatusModal(user) {
      ...
      this.userInfo = user
      this.selectedStatus = user.status || 'normal'
      this.statusReason = ''
    ...
    async updateUserStatus() {
      ...
      try {
        const res = await this.$request('user/updateStatus', {
        ...
        if (res.code === 200) {
          this.userInfo.status = this.selectedStatus
