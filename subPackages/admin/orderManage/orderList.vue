          ...
          <view class="order-info">
            ...
            <view class="info-row user-info-row">
              ...
              <view class="action-buttons">
                ...
                <!-- 只有非已取消和非已退款状态才显示退款按钮 -->
                <view class="refund-btn" v-if="order.status !== 'canceled' && order.payment_status !== 'refunded'" @click.stop="showRefundModal(order)">退款</view>
              </view>
            </view>
            <!-- 骑手信息 -->
...
export default {
  ...
  data() {
    return {
      ...
      statusOptions: [
        ...
        { label: '超时订单', value: 'timeout', count: 0 },
  ...
  methods: {
    ...
    async getZoneList() {
      try {
        ...
        if (res.code === 200 && res.data) {
          ...
          console.error('获取区域列表失败:', res.msg);
        }
      } catch (err) {
    ...
    updateZoneDisplay() {
            ...
            if (city) {
              ...
              if (this.selectedDistrict) {
                ...
                if (district) {
                  ...
                  // 设置区县ID为service_provider_id
                  zoneId = district.district_id;
                }
              } else {
                // 如果没有选择区县，则使用城市ID
                zoneId = '';
      ...
      // 更新当前选中的区域ID (service_provider_id)
      this.currentZone = zoneId;

      // 更新区域选项
      this.zoneOptions = [{ label: displayText, value: zoneId }];
    },
      ...
      try {
        ...
        if (this.currentStatus !== 'all') {
          if (this.currentStatus === 'inProgress') {
            // 进行中状态包含已分配和完成待确认
            params.status = ['assigned', 'finished'];
          } else if (this.currentStatus === 'timeout') {
            // 超时订单状态：assigned状态且time_out为true
        ...
        if (res.code === 200 && res.data) {
          ...
          if (this.currentStatus === 'timeout') {
            newOrders = newOrders.filter(order =>
              order.status === 'assigned' && order.time_out === true
            );
          }
    ...
    async confirmCancel() {
      try {
        // 检查订单是否有分配的骑手
        if (!this.currentOrder.service_member || !this.currentOrder.service_member.id) {
          uni.showToast({
            title: '该订单未分配骑手，无法撤销',
            icon: 'none'
          });
        ...
        const params = {
          ...
          service_member_id: this.currentOrder.service_member.id,
          sign: "chongchong"
        };
