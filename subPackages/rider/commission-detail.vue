  ...
  methods: {
    ...
    formatTasksData(data) {
      ...
      return data.map(item => {
        ...
        // 获取任务分配信息中的佣金
        const taskAssignment = item.task_assignment || {};
    ...
    calculateCommission(task, taskAssignment) {
      // 优先使用任务分配中的佣金字段
      if (taskAssignment.commission) {
        return taskAssignment.commission;
      }

      if (taskAssignment.amount) {
        return taskAssignment.amount;
      }
