      ...
      <view v-else>
        <view class="order-item" v-for="(order, index) in orderList" :key="index" @click="goToOrderDetail(order.id)">
          ...
          <view class="order-content">
            <view class="order-icon" :class="[order.serviceType, !order.doorImage ? 'no-image' : '']">
              <image v-if="order.doorImage" :src="order.doorImage" mode="aspectFill" class="door-image" />
              <text v-else>{{ order.serviceTypeText }}</text>
            </view>
  ...
  methods: {
        ...
        if (res.code === 200 && res.data) {
          const newOrders = res.data.map(item => {
            ...
            return {
              ...
              serviceTime: this.formatServiceTime(item.task_date, item.deadline),
    ...
    formatServiceTime(startTime, endTime) {
      ...
      try {
        ...
        const formattedEnd = this.formatTime(endDate)
