<template>
  <view class="register-page">
    ...
    <view class="register-form">
      ...
      <!-- 服务区域选择 -->
      <view class="input-group">
        <view class="input-label">选择服务区域 <text class="sub-label">(限5个)</text></view>
        <view class="input-container" @click="openServiceAreaPicker">
          <input
            type="text"
            :value="getSelectedAreaNames"
            placeholder="请选择可接单服务的城市（区县）"
            class="input"
            disabled
          />
          <text class="arrow-icon">▼</text>
     ...
     <view class="input-group">
      ...
      <button
        class="register-btn"
        :class="{ 'register-btn-active': isFormValid }"
        :style="isFormValid ? 'background: linear-gradient(135deg, #2492F2, #1a7ad9);' : ''"
    ...
    <view class="service-area-picker" v-if="showServiceAreaPicker">
      ...
      <view class="picker-content">
        <view class="picker-header">
          <text class="picker-title">选择服务区域 (最多5个)</text>
        ...
        <view class="selected-areas" v-if="selectedAreas.length > 0">
          ...
          <view class="selected-area-list">
            <view
              ...
              :key="area.district_id"
            >
              {{ area.province_name }}{{ area.city_name }}{{ area.name }}
          ...
          <view class="province-item" v-for="province in serviceAreas" :key="province.province_id">
            ...
            <view class="city-list" v-if="expandedProvinces.includes(province.province_id)">
              <view class="city-item" v-for="city in province.children" :key="city.city_id">
                <view class="city-header" @click.stop="toggleCity(city.city_id)">
                  <text class="city-name">{{ city.name }}</text>
                  <text class="expand-icon">{{ expandedCities.includes(city.city_id) ? '▼' : '▶' }}</text>
                </view>

                <view class="district-list" v-if="expandedCities.includes(city.city_id)">
                  <view
                    class="district-item"
                    v-for="district in city.children"
                    :key="district.district_id"
                    :class="{ 'district-selected': isDistrictSelected(district.district_id) }"
                    @click.stop="toggleDistrictSelection(district, province.name, city.name)"
                  >
                    <text class="district-name">{{ district.name }}</text>
                    <text class="check-icon" v-if="isDistrictSelected(district.district_id)">✓</text>
                  </view>
                </view>
              </view>
            </view>
...
export default {
  ...
  data() {
    return {
      ...
      serviceBrands: [
        ...
        { label: '怪兽', value: 'guaishou' },
        { label: '竹芒', value: 'zhumang' }
      ],
      // 推荐人ID
      referrerId: null
    }
  ...
  computed: {
    ...
    getSelectedAreaNames() {
      ...
      return this.selectedAreas.map(area => `${area.province_name}${area.city_name}${area.name}`).join('、');
    },
  ...
  methods: {
    ...
    async handleRegister() {
      ...
      if (this.formData.serviceAreas.length === 0) {
        // 用户尝试提交但没有选择服务区域，显示错误提示
        this.errors.serviceAreas = '请选择服务区域';
        uni.showToast({
          title: '请选择服务区域',
          icon: 'none'
        });
   ...
   //      service_brands: this.formData.service_brands
    ...
    toggleCity(cityId) {
      if (this.expandedCities.includes(cityId)) {
        // 如果已经展开，则收起
        this.expandedCities = this.expandedCities.filter(id => id !== cityId);
      } else {
        // 如果未展开，则展开
    ...
    isDistrictSelected(districtId) {
      return this.selectedAreas.some(area => area.district_id === districtId);
    },

    toggleDistrictSelection(district, provinceName, cityName) {
      const index = this.selectedAreas.findIndex(area => area.district_id === district.district_id);
      ...
      if (index !== -1) {
        ...
        // 如果未选中，则添加到已选中列表
        // 检查是否已经选择了5个区域
        if (this.selectedAreas.length >= 5) {
          uni.showToast({
            title: '最多只能选择5个服务区域',
            icon: 'none',
            duration: 2000
        ...
        this.selectedAreas.push(areaWithNames);
    ...
    updateFormServiceAreas() {
      if (this.selectedAreas.length > 0) {
        // 保存完整的区域信息，以便在界面上显示
        this.formData.serviceAreas = this.selectedAreas.map(area => ({
