		...
		<view class="order-card">
			...
			<view class="order-header">
				<view class="order-address">
					...
					<view class="order-time">
						<text>下单时间：{{ orderInfo.task_date }}</text>
					...
					<view class="info-content">
						<view class="order-icon">
							<text>{{ orderInfo.task_detail && orderInfo.task_detail.task_name ? orderInfo.task_detail.task_name.charAt(0) : '补' }}</text>
						</view>
						<view class="info-right">
							<view class="order-title">
								【{{ orderInfo.task_detail && orderInfo.task_detail.task_name ? orderInfo.task_detail.task_name : '充充跑腿' }}】充充跑腿订单
							</view>
							<view class="address-text">{{ orderInfo.task_detail.store_name }}</view>
							<view class="contact-info">
								<text class="contact-name ">{{ orderInfo.name }}</text>
								<text class="contact-phone">{{ orderInfo.phone_number }}</text>
			...
			<view class="order-info-list">
				...
				<view class="info-item">
					...
					<text class="value">{{ displayBrand }}</text>
				</view>
				<view class="divider"></view>
				<view class="info-item">
					<text class="label">主项服务：</text>
					<text class="value">
						<text class="task-name">{{ orderInfo.task_detail && orderInfo.task_detail.task_name ? orderInfo.task_detail.task_name : '' }}</text>
						<text class="item-number">x{{ orderInfo.task_detail && orderInfo.task_detail.item_number ? orderInfo.task_detail.item_number : '' }}</text>
					</text>
				</view>
				<view class="divider"></view>
				<!-- <view class="info-item">
				...
				<view class="info-item">
					...
					<text
						class="value ">{{ formatServiceTime(orderInfo.start_date, orderInfo.deadline) }}</text>
				</view>
				<!-- 建议骑手上门时间段 -->
	...
	export default {
		...
		methods: {
			...
			calculateCountdown() {
				...
				try {
					// 将时间字符串转换为时间戳
					const deadline = new Date(this.orderInfo.deadline.replace(/-/g, '/')).getTime()
