					...
					<view class="service-options">
						...
						<view class="quantity-selector">
							<view class="quantity-controls">
								...
								<input type="number" v-model="formData.quantity" class="quantity-input" />
	...
	export default {
		...
		methods: {
			...
			calculatePrice() {
				...
				// 1. 计算基础补宝服务费用
				let baseServiceFee = parseFloat(info.bubao_base_device_fee) || 0; // 初始为基础设备费用
				let extraDeviceFee = 0;

				// 如果设备数量超过基础数量，计算额外费用
				if (this.formData.quantity > info.bubao_base_device) {
					const extraDevices = Math.ceil((this.formData.quantity - info.bubao_base_device) / info.bubao_extra_device);
