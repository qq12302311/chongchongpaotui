            ...
            <view class="filter-row">
              ...
              <view class="filter-items">
                <view class="sort-options">
                  <view
                    class="sort-item"
                    :class="{ active: currentSortField === 'total_commission' }"
                    @click="toggleSort('total_commission')"
                  >
                    总收入{{ getSortIcon('total_commission') }}
                  </view>
                  <view
                    class="sort-item"
                    :class="{ active: currentSortField === 'total_completed_tasks_count' }"
                    @click="toggleSort('total_completed_tasks_count')"
                  >
                    总接单{{ getSortIcon('total_completed_tasks_count') }}
                  </view>
                </view>
              </view>
            </view>
      ...
      <view v-else class="rider-list-content">
        ...
        >
            ...
            <view class="rider-info">
              <view class="rider-name-row">
                <view class="rider-name">
				  ...
				  <view class="checkbox-wrapper" @click.stop="toggleSelect(rider.phone_number)">
				  	<checkbox-group>
				  		<checkbox :checked="selectedIds.includes(rider.phone_number)" color="#2492F2" style="transform: scale(0.8);" />
				  	</checkbox-group>
				  </view>
                </view>
          ...
          <view class="rider-details">
            ...
            <!-- 总收入和总接单 -->
            <view class="detail-item commission-stats-item">
              <view class="commission-stats-left">
                <text class="detail-label">总收入:</text>
                <text class="detail-value commission-value">¥{{ rider.total_commission || '0.00' }}</text>
              </view>
              <view class="total-orders-wrapper">
                <text class="detail-label">总接单:</text>
                <text class="detail-value orders-value">{{ rider.total_completed_tasks_count || '0' }}单</text>
              </view>
            </view>
            ...
            <view class="detail-item service-zone-item" style="width: 100%;">
              ...
              <view class="service-zone-wrapper">
                <text v-if="!rider.service_city || rider.service_city.length === 0" class="detail-value cert-none">未设置</text>
                <view v-else class="service-zone-cities">
                  <!-- 直接显示service_city数组中的城市 -->
                  <view class="city-tags-wrapper">
                    <!-- 未展开状态：显示前3个城市 -->
                    <template v-if="!isZoneExpanded(rider.service_member_id)">
                      <text
                        v-for="(city, index) in getDisplayServiceCities(rider.service_city, 3)"
                        :key="index"
                        class="city-zone-tag"
                      >
                        {{ city }}
                      ...
                      <!-- 展开按钮 -->
                      <view
                        v-if="rider.service_city.length > 3"
                        class="zone-expand-btn"
                        @click.stop="toggleZoneExpand(rider.service_member_id)"
                      >
                        <text class="zone-count">查看全部{{ rider.service_city.length }}个</text>
                    ...
                    <template v-else>
                      <text
                        v-for="(city, index) in rider.service_city"
                        :key="index"
                        class="city-zone-tag"
                      >
                        {{ city }}
...
export default {
  ...
  data() {
    return {
      ...
      currentSortOrder: '', // 当前排序方向 (desc -> asc -> '' 循环)
      // 服务区域筛选
      ...
      expandedCityZones: {}, // 存储已展开城市的骑手ID和城市名，格式: { 'riderId': ['成都市', '北京市'] }
  ...
  computed: {
  	isAllSelected() {
  		const pendingList = this.riderList;
  ...
  methods: {
	  ...
	  toggleSelectAll() {
		if (this.isAllSelected) {
			...
			this.selectedIds = this.riderList.map(w => w.phone_number);
		}
	  },
	  // 批量同意
	  batchApprove() {
    ...
    async getRiderList() {
      ...
      try {
        ...
        if (this.currentSortField && this.currentSortOrder) {
          params.sort_field = this.currentSortField;
    ...
    // 获取要显示的服务城市（限制数量）
    getDisplayServiceCities(serviceCities, limit) {
      if (!serviceCities || !Array.isArray(serviceCities)) {
        return [];
      }
      return serviceCities.slice(0, limit);
    },

    // 切换地级市区县展开状态
    toggleCityZoneExpand(riderId, cityName) {
      if (!this.expandedCityZones[riderId]) {
        this.$set(this.expandedCityZones, riderId, []);
      }

      const expandedCities = this.expandedCityZones[riderId];
      const index = expandedCities.indexOf(cityName);

      if (index > -1) {
        // 如果已经展开，则收起
        expandedCities.splice(index, 1);
      } else {
        // 如果未展开，则展开
    ...
    // 检查地级市区县是否展开
    isCityZoneExpanded(riderId, cityName) {
      return this.expandedCityZones[riderId] &&
             this.expandedCityZones[riderId].includes(cityName);
    },
...
.sort-icon {
  width: 32rpx;
  height: 32rpx;
  background-image: url('data:image/svg+xml;base64,');
...
.detail-item {
  ...
  &.commission-stats-item {
    width: 100% !important; /* 总收入和总接单项目占满整行 */
    justify-content: space-between;
    margin-top: 8rpx;

    .commission-stats-left {
      display: flex;
      align-items: center;
    }
